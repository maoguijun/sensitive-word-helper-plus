...
class Mint extends core_1.Tree {
    ...
    _filterFn(word, every = false, replace = true) {
        ...
        for (endIndex; endIndex <= wordLen; endIndex++) {
            ...
            console.log('key:', key, 'originalKey:', originalKey, 'judgeText:', judgeText, 'currNode:', currNode, 'prevNode:', prevNode, 'spareText:', spareText);
            if (isJudge && currNode) {
                if (replace) {
                    if (spareText) {
                    }
...
if (require.main === module) {
    ...
    // ['bd', 'db'] 1bddb2
    let m = new Mint({ keywords: ['淘宝', '拼多多', '京东', 'TEST'] });
